...
use Drupal\Driver\Remote\Exception\FilterFormatException;
use Drupal\Driver\Remote\Exception\RuntimeException;
...
class Node extends BaseDrupalRemoteAPI {
    ...
    {
        if (!in_array('Drupal\Driver\Remote\CustomFormatterInterface', class_implements($this->custom_formatter))) {
            throw new \Exception(sprintf('Custom formatter class %s must implement Drupal\Driver\Remote\CustomFormatterInterface.', get_class($this->custom_formatter)));
        }
    }
  ...
  /**
   * @return mixed
   * @throws \Drupal\Driver\Remote\Exception\FilterFormatException
   *
